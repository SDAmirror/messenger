...
def connection_processor(client, id, cryptor,logger):
    try:
        ...
        if data['authentification_check']:
            ...
            if data['connection_case'] == 'registration':
                ...
                if not email_validation(user.email):
                    client.send(
                        json.dumps({"message": "registration failed: wrong email format", "auth_succses": False}).encode())
                    return flag_processor_success
                    ...
                    if validation_res == True:
                        ...
                        if 'username_taken' in ress['errors']:
                            client.send(json.dumps(
                                {"message": "registration failed: username already taken", "auth_succses": False}).encode())
                        if ress['created']:
